function [E_BestQuantile,D_BestQuantile,GeneralEQbias] = WithinSubjectDistRL_InnerLoop(thisPdata,resolution,epsilon)
...
%                  if Qtbl(ChosenType(t),ChosenProb(t)) == Qtbl(UnChosenType(t),UnChosenProb(t))
%                      Qchoice(t,1) = trialoptions(randi(2));
%                  else
%                      
%                      if Qtbl(ChosenType(t),ChosenProb(t)) > Qtbl(UnChosenType(t),UnChosenProb(t))
%                          Qchoice(t,1) = trialoptions(1);
%                      else
%                           Qchoice(t,1) = trialoptions(2);
%                      end
%                  end % of letting the algorithm make a choice
%                  
                ...
                ChosenQ(ctr,t) = Qtbl(ChosenType(t),ChosenProb(t));
                UnChosenQ(ctr,t) = Qtbl(UnChosenType(t),UnChosenProb(t));
                
                Q_D20(ctr,t) = Qtbl(1,1);
                Q_D50(ctr,t) = Qtbl(1,2);
                Q_D80(ctr,t) = Qtbl(1,3);
                
                Q_E20(ctr,t) = Qtbl(2,1);
                Q_E50(ctr,t) = Qtbl(2,2);
                Q_E80(ctr,t) = Qtbl(2,3);
    ...
    [BestAcc(1,1),quantIX] = max(Acc);
    BestQuantile(1,1) = distQuantile(quantIX);
    
   [E_BestAcc(1,1),E_quantIX] = max(EAcc);
    E_BestQuantile(1,1) = distQuantile(E_quantIX);
    
   [D_BestAcc(1,1),D_quantIX] = max(DAcc);
    D_BestQuantile(1,1) = distQuantile(D_quantIX);
    
    
    % what was the general equiprobable bias across all EQ conditions?
    GeneralEQbias(1,1) = mean(pickedExp(EQ_ix));
