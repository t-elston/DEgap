function [xx] = WithinSubjectPredictEQfromTraining_v01(wPdata,Within_AllData)
xx=[];

Data = wPdata.Choice;
VarNames =  Data.Properties.VariableNames;
Loss_ix = Data.context==2;

% rectify the loss data 
Data = table2array(Data);

Data(:,22:24) = abs(.5 - Data(:,22:24));
...
[MeanTrainx20,EQx20Bias,ContextF,SubjectF]  = GetTrainPerfByProbWithinSubject_v01(Within_AllData,.2);
[MeanTrainx50,EQx50Bias]                    = GetTrainPerfByProbWithinSubject_v01(Within_AllData,.5);
[MeanTrainx80,EQx80Bias]                    = GetTrainPerfByProbWithinSubject_v01(Within_AllData,.8);
...
lme_tbl.Subject   = [SubjectF ; SubjectF ; SubjectF ];
...
tbl.DTrainPerf = nanmean(table2array(Data(:,4:6)),2);
tbl.ETrainPerf = nanmean(table2array(Data(:,7:9)),2);
tbl.MeanTrain  = nanmean(table2array(Data(:,4:9)),2);
tbl.EQperf     = nanmean(table2array(Data(:,22:24)),2);
tbl.Subject    = repmat([1:60]',2,1);

[DxE_x_EQ_mdl] = fitlme(tbl,'EQperf ~ 1 + MeanTrain + Protocol + MeanTrain:Protocol + (1|Subject)'); 
...
[DTrain_x_EQ_mdl] = fitlme(tbl,'EQperf ~ 1 + DTrainPerf + Protocol + DTrainPerf*Protocol + (1|Subject)'); 
...
[ETrain_x_EQ_mdl] = fitlme(tbl,'EQperf ~ 1 + ETrainPerf + Protocol + ETrainPerf*Protocol + (1|Subject)'); 
